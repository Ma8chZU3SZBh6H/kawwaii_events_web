...
<div class="flex flex-col gap-3">
        ...
        <div class="grid md:grid-cols-3 gap-6 relative bg-bgTrans md:bg-transparent rounded-md ">
            ...
            <div class="flex flex-col gap-6 relative z-10 p-4">
                ...
                <div class="flex flex-col gap-3">
                    <p class="text-lightTitle md:text-title  text-lg">Registration</p>
                    @if ($joint != null)
                    <form class="flex flex-col gap-3" action="{{route('join.auth.leave', $event)}}" method="post">
                        @csrf
                        <div>
                            <input class="bg-transparent cursor-pointer text-lightText border-lightText md:text-text md:border-text border rounded py-1 px-4" type="submit" value="Leave?">
                        </div>
                    </form>
                    @else
    ...
    <div>
        ...
        @if (($joined_people ?? null) != null)
        <x-title>Joined people</x-title>
        @foreach ($joined_people as $person)
        <div class="flex flex-row justify-between">
            <x-text>{{$person->email}}</x-text>
            <div>
                <form class="flex flex-col gap-3" action="{{route('join.auth.remove', $event)}}" method="post">
                    @csrf
                    <input type="hidden" name="email" value="{{$person->email}}">
                    <div>
                        <input class="bg-transparent cursor-pointer text-text border-text border rounded  px-2" type="submit" value="Remove">
                    </div>
                </form>
            </div>
        </div>
